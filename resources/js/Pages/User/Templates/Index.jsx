...
export default function TemplatesIndex({ initialTemplates, filters, total }) {
        ...
        {loading ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {[...Array(itemsPerPage)].map((_, i) => (
              <div key={i} className="animate-pulse bg-gray-100 rounded-xl h-80 flex flex-col overflow-hidden">
                {/* صورة وهمية */}
                <div className="h-32 w-full bg-blue-200 mb-4 flex items-center justify-center">
          ...
          <>
            ...
            {templates.map(template => (
                <Card key={template.id} className="flex flex-col h-80 hover:shadow-lg transition-shadow border border-gray-100 rounded-xl overflow-hidden">
                  {/* صورة template */}
                  ...
                  <CardHeader className="p-4 flex-1">
                    <CardTitle className="text-lg font-bold text-gray-800 mb-1 line-clamp-1">{template.title}</CardTitle>
                    <p className="text-gray-600 text-sm line-clamp-2 mb-2">{template.description}</p>
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <span className="px-2.5 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">
                        {template.type === 'report' ? 'تقرير' : 
                         template.type === 'survey' ? 'استطلاع' : 
                         template.type === 'assessment' ? 'تقييم' : template.type}
                      </span>
                    </div>
                  </CardHeader>
                  <div className="p-4 mt-auto">
                    <Link href={route('user.templates.show', template.id)} className="w-full block">
